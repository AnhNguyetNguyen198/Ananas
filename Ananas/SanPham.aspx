<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeFile="SanPham.aspx.cs" Inherits="Ananas.SanPham" %>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        <div class="product-container">
            <asp:DataList ID="DataListSP" runat="server" DataSourceID="SqlDataSourceSanPham" 
                          RepeatColumns="3" RepeatDirection="Horizontal" class="product-list" 
                          OnItemDataBound="DataListSP_ItemDataBound">
                <ItemTemplate>
 ...
 <asp:HyperLink ID="HyperLink1" runat="server" NavigateUrl='<%# "~/ChonSanPham.aspx?MaSanPham=" + Eval("MaSanPham") %>' Text='<%# Eval("TenSanPham") %>' Font-Bold="True" Font-Italic="False" Font-Size="13pt" Font-Underline="False" ForeColor="Black"></asp:HyperLink>
    ...
    CASE 
        ...
        THEN p.Gia * (1 - km.TyLeKhuyenMai / 100.0)  -- Tính giá sau khuyến mãi nếu có khuyến mãi hiệu lực
        ELSE p.Gia
    END AS PriceAfterDiscount,
    CASE 
        WHEN km.MaKhuyenMai IS NOT NULL 
             AND GETDATE() BETWEEN km.NgayBatDau AND km.NgayKetThuc
        THEN 1  -- Sản phẩm có khuyến mãi
        ELSE 0   -- Không có khuyến mãi
    END AS IsPromo
FROM 
    SanPham p
LEFT JOIN 
...
WHERE 
    (km.MaKhuyenMai IS NULL  -- Sản phẩm không có khuyến mãi
    OR GETDATE() &lt; km.NgayBatDau  -- Sản phẩm có khuyến mãi nhưng chưa đến ngày bắt đầu
    OR (GETDATE() BETWEEN km.NgayBatDau AND km.NgayKetThuc))  -- Sản phẩm có khuyến mãi và khuyến mãi còn hiệu lực
